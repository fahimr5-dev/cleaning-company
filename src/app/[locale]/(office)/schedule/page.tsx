import { setRequestLocale } from "next-intl/server";
import { TodoScreen } from "@/components/common/todo-screen";
import { requireRole } from "@/lib/auth";

/**
 * NOT BUILT YET — delivered in Phase 3.
 *
 * The role check below is real and active today: someone who is not allowed
 * here is redirected to their own home screen before this page renders. What
 * is missing is the feature itself, which is why this shows a placeholder
 * rather than an empty table pretending to be a working screen.
 */
export default async function Page({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  setRequestLocale(locale);
  await requireRole(locale, "OWNER", "OPS_MANAGER");

  return (
    <TodoScreen
      feature="The drag-and-drop schedule"
      phase={3}
      willInclude={[
        "Weekly calendar with teams as rows and jobs as draggable blocks",
        "Colour by status: scheduled, en route, in progress, completed, cancelled, no-access",
        "Conflict detection for double-booking and unrealistic travel time between zones",
        "Recurring job engine: skip or move one visit without breaking the series",
        "Capacity view showing utilisation per team per day",
      ]}
    />
  );
}
