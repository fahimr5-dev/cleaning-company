...
/** Search and filter the staff list. The filters live in the address bar. */
export function StaffFilters({ total }: { total: number }) {
  ...
  const compliance = params.get("compliance") ?? "ALL";
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-muted-foreground text-xs">{t("filters.papers")}</span>
        {(["ALL", "PROBLEM", "EXPIRED", "EXPIRING", "VALID"] as const).map((value) => (
          <button
            key={value}
            type="button"
            data-testid={`staff-filter-${value}`}
            className={chip(compliance === value)}
            onClick={() => setFilter("compliance", value)}
          >
            {t(`filters.papersValues.${value}`)}
          </button>
        ))}

        <span className="text-muted-foreground ms-3 text-xs">{t("filters.status")}</span>
        {(["ALL", "ACTIVE", "ON_LEAVE", "TERMINATED"] as const).map((value) => (
          <button
            key={value}
            type="button"
